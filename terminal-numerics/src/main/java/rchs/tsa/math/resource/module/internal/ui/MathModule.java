...
public class MathModule extends AbstractModule implements IModuleDelegate
{
	...
	public void setupMathData(MathData data, Properties props)
	{
		...
		data.addFunction("sin", (n) -> Math.sin(n));
		data.addFunction("cos", (n) -> Math.cos(n));
		data.addFunction("tan", (n) -> Math.tan(n));
		data.addFunction("asin", (n) -> Math.asin(n));
		data.addFunction("acos", (n) -> Math.acos(n));
		data.addFunction("atan", (n) -> Math.atan(n));
		data.addFunction("csc", (n) -> 1 / Math.sin(n));
		data.addFunction("sec", (n) -> 1 / Math.cos(n));
		data.addFunction("cot", (n) -> 1 / Math.tan(n));
